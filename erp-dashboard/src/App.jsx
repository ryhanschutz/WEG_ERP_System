...
const tdS = (x={}) => ({
  ...
  fontSize:"0.85rem", color:"#111111", ...x,
});
    ...
    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
      <div style={{ flex:1, height:4, background:"#e0e0e0", borderRadius:1 }}>
        <div style={{ width:`${p}%`, height:"100%",
          background:p>=100?"#1a7a3c":T.azul, transition:"width 0.4s" }} />
      </div>
      <span style={{ fontSize:"0.8rem", color:T.cinza, minWidth:32, textAlign:"right" }}>
        {Math.round(p)}%
      </span>
    </div>
  );
...
function LoginScreen({ onLogin }) {
  ...
  const fi = {
    width:"100%", fontFamily:T.fonte, fontSize:"0.95rem", padding:"10px 12px",
    border:"1px solid #ccc", borderRadius:2, outline:"none",
    background:T.branco, color:T.preto, boxSizing:"border-box",
  };
  ...
  return (
    <div style={{ borderBottom:"1px solid #e0e0e0", paddingBottom:20, marginBottom:20 }}>
        ...
        {[
          ...
          <div key={x.l} style={{ background:T.cinzaClaro, padding:"10px 14px", borderRadius:2,
            ...
            <p style={{ fontSize:"1.4rem", fontWeight:700, color:x.w?"#b91c1c":T.preto, lineHeight:1 }}>
              {x.v}
              {x.u && <span style={{ fontSize:"0.75rem", fontWeight:400, color:T.cinza, marginLeft:4 }}>{x.u}</span>}
            </p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        ...
        {m.status==="produzindo" && <>
          <Btn v="s" onClick={()=>onCmd(m.id,"pausar")}>Pausar</Btn>
          <Btn v="d" onClick={()=>onCmd(m.id,"encerrar")}>Encerrar</Btn>
        </>}
        {m.status==="pausada"   && <>
          <Btn onClick={()=>onCmd(m.id,"iniciar")}>Retomar</Btn>
          <Btn v="d" onClick={()=>onCmd(m.id,"encerrar")}>Encerrar</Btn>
        </>}
...
function ModalOrdem({ onClose, onSave }) {
  ...
  const fi = {
    width:"100%", fontFamily:T.fonte, fontSize:"0.9rem", padding:"8px 10px",
    border:"1px solid #ccc", borderRadius:2, background:T.branco,
    color:T.preto, outline:"none", boxSizing:"border-box",
  };
  return (
...
// ─── MÓDULO ERP ───────────────────────────────────────────────────────────────
function ModuloERP({ user, machines, setMachines, ordens, setOrdens, estoque, log, addLog }) {
  const [modal, setModal] = useState(false);

  // Polling de máquinas e ordens a cada 3s
  useEffect(() => {
    const iv = setInterval(async () => {
      try {
        const [resMaq, resOrd] = await Promise.all([
          fetch("/api/maquinas"),
          fetch("/api/ordens"),
        ]);
        if (resMaq.ok)  setMachines(await resMaq.json());
        if (resOrd.ok)  setOrdens(await resOrd.json());
      } catch (e) {
        console.error("Erro no polling:", e);
      }
    }, 3000);
    return () => clearInterval(iv);
  }, [setMachines, setOrdens]);

  // Envio de comando via API → Java → MQTT → ESP32
  const handleCmd = useCallback(async (mid, cmd) => {
    try {
      await fetch("/api/comandos", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ maquina_id: mid, comando: cmd, autor: user.nome }),
      });
      addLog("info", `[${user.nome}] Comando '${cmd}' enviado → ${mid}.`);
    } catch (e) {
      addLog("error", `Erro ao enviar comando: ${e.message}`);
    }
  }, [user, addLog]);
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:40 }}>
        <div>
          ...
          <section>
            <H2 n="4">Estoque</H2>
            {estoque.length === 0
              ? <p style={{ fontSize:"0.9rem", color:T.cinza }}>Carregando estoque...</p>
              : <table style={{ width:"100%" }}>
                  <thead><tr>
...
function ModuloPLM({ user, addLog }) {
  ...
  // Busca produtos e fases do banco
  useEffect(() => {
    const carregar = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setProdutos(data);
          if (data.length > 0) setSel(data[0].id);
        } else {
          addLog("warning", "Erro ao carregar dados PLM do servidor.");
        }
      } catch (e) {
        addLog("warning", "Erro ao carregar PLM: " + e.message);
      } finally {
        setCarregando(false);
      }
    };
  ...
  const abrirEdicao = (prodId, faseIdx, fase) => {
    setEditando({ prodId, faseIdx, faseId: fase.id });
    setNovoStatus(fase.status);
    setNovaObs(fase.obs || "");
  };

  const salvarFase = async () => {
    try {
      const res = await fetch(`/api/plm/fase/${editando.faseId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status:       novoStatus,
      ...
      if (res.ok) {
        const nomeFase = prod.fases[editando.faseIdx].nome;
        // Atualiza estado local
        setProdutos(prev => prev.map(p => p.id !== editando.prodId ? p : {
          ...p,
          fases: p.fases.map((f, i) => i !== editando.faseIdx ? f : {
            ...f,
            status:      novoStatus,
            obs:         novaObs,
        ...
        addLog("error", "Erro ao salvar fase PLM.");
      }
    } catch (e) {
      addLog("error", "Erro ao salvar fase: " + e.message);
    }
    setEditando(null);
  };

  if (carregando) {
    return <p style={{ fontSize:"0.9rem", color:T.cinza, lineHeight:1.7 }}>Carregando dados PLM...</p>;
  }
  ...
  return (
    <div>
      ...
      <section>
        ...
        {produtos.length === 0 ? (
          <p style={{ fontSize:"0.9rem", color:T.cinza }}>Nenhum produto encontrado no banco.</p>
        ) : (
          <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:32 }}>
                  ...
                  <table style={{ width:"100%" }}>
                    <tbody>
                      {[
                        ...
                        ["Peso",        prod.peso],
                        ["Tolerância",  prod.tolerancia],
                        ["Responsável", prod.responsavel],
                        ["Revisão",     prod.revisao],
                      ].map(([k,v], i) => (
                ...
                <div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", marginBottom:20 }}>
                    {prod.fases.map((f, i) => (
                      <div key={f.id} style={{ display:"flex", alignItems:"center", flex:1 }}>
                        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flex:1 }}>
                          <div style={{ width:28, height:28, borderRadius:"50%",
                            ...
                            color:FASE_COLOR[f.status]||T.cinza, marginBottom:4 }}>
                            {i+1}
                          </div>
                  ...
                  {/* Fases detalhadas */}
                  {prod.fases.map((f, i) => (
                    <div key={f.id} style={{ background:FASE_BG[f.status],
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between",
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                          <span style={{ fontSize:"0.72rem", fontWeight:600, textTransform:"uppercase",
                            letterSpacing:"0.04em", color:FASE_COLOR[f.status]||T.cinza }}>
                            {f.status}
                          </span>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)",
          ...
          <div style={{ background:T.branco, borderTop:"3px solid #1a7a3c", borderRadius:2,
            ...
            <h2 style={{ fontSize:"1.1rem", fontWeight:700, marginBottom:20, marginTop:0 }}>
              Atualizar fase: {prod?.fases[editando.faseIdx]?.nome}
            </h2>
            <div style={{ marginBottom:16 }}>
              ...
              <select value={novoStatus} onChange={e=>setNovoStatus(e.target.value)}
                style={{ width:"100%", fontFamily:T.fonte, fontSize:"0.9rem", padding:"8px 10px",
                  border:"1px solid #ccc", borderRadius:2, outline:"none" }}>
                {["pendente","em andamento","aprovado","reprovado"].map(s =>
            ...
            <p style={{ fontSize:"0.75rem", color:T.cinza, marginBottom:16 }}>
              Alteração registrada por:{" "}
              <span style={{ fontWeight:600, color:T.preto }}>{user.nome}</span>
            </p>
...
export default function App() {
  ...
  // Carga inicial ao fazer login
  useEffect(() => {
    if (!user) return;
    const carregar = async () => {
      try {
        const [resMaq, resOrd, resEst] = await Promise.all([
          fetch("/api/maquinas"),
          fetch("/api/ordens"),
          fetch("/api/estoque"),
        ]);
        ...
        addLog("info", "Dados carregados do servidor.");
      } catch (e) {
        addLog("warning", "Erro ao carregar dados: " + e.message);
      }
    };
    carregar();
  }, [user, addLog]);
      ...
      <div style={{ minHeight:"100vh", background:"#f8f8f8",
        ...
        <div style={{ width:"100%", maxWidth:1100, background:T.branco,
          ...
          <nav style={{ borderBottom:"1px solid #e0e0e0", padding:"0 40px", display:"flex" }}>
            {[
              { id:"erp", label:"ERP — Produção" },
              { id:"plm", label:"PLM — Ciclo de Vida" },
            ].map(a => (
